...
func _ready():
	...
	pos = position
	$idle_timer.wait_time = rng.randf_range(2.5, 3.5)
...
func _physics_process(delta):
	...
	if health <= 0:
		...
		$aggro_timer.stop()
		origin = false
		if left and abs(player.position + Vector2(10, -20) - position) > Vector2(1,1):
			position += (player.position + Vector2(10, -20) - position).normalized() * 0.9
		elif right and abs(player.position + Vector2(-10, -20) - position) > Vector2(1,1):
			position += (player.position + Vector2(-10, -20) - position).normalized() * 0.9
		$AnimatedSprite2D.play("walk")
...
func attack():
	if player_chase and can_deal_damage and (strike_dir == "right" or strike_dir == "left"):
		...
		if strike_dir == "right":
			...
			$AnimatedSprite2D.flip_h = true
			$AnimatedSprite2D.play("attack")
	if !dealt_dmg and attacking and $AnimatedSprite2D.frame == 3 and ((strike_dir == "right" and $AnimatedSprite2D.flip_h == false) or (strike_dir == "left" and $AnimatedSprite2D.flip_h == true)) :
		dealt_dmg = true
