...
func _ready():
	...
	pos = position
	$idle_timer.wait_time = rng.randf_range(5, 6)
...
func _physics_process(delta):
	...
	if health <= 0:
		...
		$aggro_timer.stop()
		origin = false
		if left and abs(player.position + Vector2(10, 10) - position) > Vector2(1,1):
			position += (player.position + Vector2(10, 10) - position).normalized() * 0.5
		elif right and abs(player.position + Vector2(-10, 10) - position) > Vector2(1,1):
			position += (player.position + Vector2(-10, 10) - position).normalized() * 0.5
		$AnimatedSprite2D.play("walk")
